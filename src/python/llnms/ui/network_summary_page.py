...
def print_network_summary_table( stdscr, llnms_state, rmin ):
	...
	table.setColumnHeaderItem( 2, 'Responsive', 0.09 )
	table.setColumnHeaderItem( 3, 'Last Modified', 0.2)
	
	table.setColumnAlignment( 0, CursesTable.ALIGN_LEFT );
	table.setColumnAlignment( 1, CursesTable.ALIGN_LEFT );
	table.setColumnAlignment( 2, CursesTable.ALIGN_MIDDLE );
	table.setColumnAlignment( 3, CursesTable.ALIGN_LEFT );
	...
	for x in xrange( 0, len(llnms_state.network_status.network_assets)):
		...
		table.setItem( 2, x+1, str(llnms_state.network_status.network_assets[x].respond_ping) )

		#  Set the modified date
		table.setItem( 3, x+1, str(llnms_state.network_status.network_assets[x].date_scanned) )
