...
llnms_add_registered_scanner_to_asset(){
    ...
    SCANNER_CNT=`$LLNMS_HOME/bin/llnms-print-asset-info  -f $ASSET_PATH -s | sed '/^\s*$/d' | wc -l | sed 's/ *//g'`
    ...
    # Add the id
    xmlstarlet ed -L --subnode "/llnms-asset/scanners/scanner[`expr $SCANNER_CNT + 1`]" --type elem -n 'id' -v "`$LLNMS_HOME/bin/llnms-print-scanner-info  -f $SCANNER_PATH --id`" $ASSET_PATH
    ...
    for ((x=1; x<=$NUM_ARGS; x++)); do
        ...
        if [ "$SCANNER_ARG_TYPE" = 'ASSET_ELEMENT' ]; then
            ARG_VALUE=`$LLNMS_HOME/bin/llnms-print-asset-info  -f $ASSET_PATH "--${SCANNER_ARG_VALUE}"`
...
ASSET_PATHS=`$LLNMS_HOME/bin/llnms-list-assets -path -l`
ASSET_EXISTS=0
for ASSET_FILE in $ASSET_PATHS; do
   
    # compare hostname
    if [ "$ASSET_VALUE" = "`$LLNMS_HOME/bin/llnms-print-asset-info -f $ASSET_FILE -host`" ]; then
        ASSET_EXISTS=1
...
REG_SCANNERS=`$LLNMS_HOME/bin/llnms-print-asset-info  -f $ASSET_PATH -s`
